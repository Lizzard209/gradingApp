	...
	public static void main(String[] args) 
	{
		...
		for(int row= 0; row < grades.length; row++)
			for(int col= 0;col < grades[row].length;col++ ) 
				if (col<4) {
					System.out.println("Enter the grades for exam " + (col+1) + " for student " + (row+1) + " (Must be between 0 and 50)");
				grades[row][col]= scan.nextInt();
				}
				else {
					int assignmentNumber = (col-3);
					System.out.println("Enter the grades for assignment " + assignmentNumber + " for student " + (row+1) + " (Must be between 0 and 75)");
		...
		for(int row=0; row<grades.length;row++)
		{
			...
			for(int i=0; i<studentSum.length;i++) {
				...
				if (studentSum[i] >=400 && studentSum[i]<450) {
					letterGrade = "B"; 
				}
				if (studentSum[i] >=350 && studentSum[i]<400) {
					letterGrade = "C"; 
				}
				if (studentSum[i] >=300 && studentSum[i]<350) {
					letterGrade = "D"; 
				}
				if (studentSum[i]<300) {letterGrade = "F"; 
...
public static void calculateSD(double studentSum[])
{
    ...
    for(int i=0; i<studentSum.length;i++) {
		...
		if (studentSum[i] >=(mean + 1*SD) && (studentSum[i]<(mean +2*SD))) {
			letterGrade = "B"; 
		}
		if (studentSum[i] >=(mean) && (studentSum[i]<(mean +1*SD))) {
			letterGrade = "C"; 
		}
		if (studentSum[i] >=(mean - 1*SD) && (studentSum[i]<(mean))) {
			letterGrade = "D"; 
		}
		if (studentSum[i]<300) {letterGrade = "F"; 
